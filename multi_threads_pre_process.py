...
def build_wordmap_zh():
    ...
    word_freq_pk = 'data/word_freq.pk'
    if os.path.exists(word_freq_pk):
        f = open(word_freq_pk, 'rb')
        ...
        f = open(word_freq_pk, 'wb')
        pickle.dump(word_freq, f)
        f.close()

    # Create word map
    # words = [w for w in word_freq.keys() if word_freq[w] > min_word_freq]
    words = word_freq.most_common(output_lang_vocab_size - 4)  # 常见的5000 - 4 个词
        ...
        with Manager() as manager:
            ...
            for i in range(cpu_nums):
                if i == cpu_nums-1:
                    ...
                    en = data_en[i*batchs: (i+1)*batchs]
                    zh = data_zh[i*batchs: (i+1)*batchs]
                p = Process(target=mp_func, args=(en, zh, d, i))
                p.start()
                p_list.append(p)
            for res in p_list:
                res.join()
